...
class TestStorageIntegration:
    ...
    def test_service_health(self, storage_api_base_url):
        ...
        assert wait_for_service(storage_api_base_url), "Storage service not ready"
    ...
    def test_minio_connectivity(self, minio_client, test_bucket_name):
        ...
        try:
            ...
            upload_test_file(minio_client, bucket_name, object_name, test_content)
    ...
    def test_storage_upload_download_flow(self, storage_api_base_url, auth_headers):
    ...
    def test_storage_directory_operations(self, storage_api_base_url, auth_headers):
        ...
        for item in list_response['items']:
            if item['name'] == 'test_directory' and item['type'] == 'directory':
                directory_found = True
                break
        
        assert directory_found, "Created directory not found in listing"
    
    def test_storage_file_deletion(self, storage_api_base_url, auth_headers, minio_client, test_bucket_name):
        ...
        from .conftest import upload_test_file, assert_file_exists_in_minio, assert_file_not_exists_in_minio
        upload_test_file(minio_client, test_bucket_name, object_name, test_content)
        
        # Verify file exists
        assert assert_file_exists_in_minio(minio_client, test_bucket_name, object_name)
        ...
        assert assert_file_not_exists_in_minio(minio_client, test_bucket_name, object_name)
...
class TestStoragePerformance:
    ...
    @pytest.mark.slow
    def test_multiple_upload_urls_performance(self, storage_api_base_url, auth_headers):
        ...
        assert avg_time_per_url < 0.1, f"Average time per URL generation: {avg_time_per_url:.3f}s"
        print(f"Generated 50 upload URLs in {total_time:.2f}s (avg: {avg_time_per_url:.3f}s per URL)")
    
    @pytest.mark.slow
    def test_large_directory_listing(self, storage_api_base_url, auth_headers, minio_client, test_bucket_name):
        ...
        for i in range(file_count):
            object_name = f"project_1/integration_test_large_dir/file_{i:03d}.txt"
            content = f"Content of file {i}"
            upload_test_file(minio_client, test_bucket_name, object_name, content)
        ...
        list_params = {
            'project_id': 1,
            'path': '/integration_test_large_dir'
        }
        ...
        assert listing_time < 2.0, f"Directory listing took {listing_time:.2f}s for {file_count} files"
