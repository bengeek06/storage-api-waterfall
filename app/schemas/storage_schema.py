...
def validate_path(value):
    ...
    if ".." in value or value.startswith("/"):
        raise ValidationError("Path cannot contain '..' sequences or start with '/'")
    return value.strip()
...
class StorageFileSchema(SQLAlchemyAutoSchema):
    ...
    bucket_type = fields.String(required=True, validate=lambda x: x in VALID_BUCKET_TYPES)
...
class FileVersionSchema(SQLAlchemyAutoSchema):
    ...
    created_at = fields.DateTime(dump_only=True, format="iso")
    updated_at = fields.DateTime(dump_only=True, format="iso")
    validated_at = fields.DateTime(dump_only=True, format="iso", allow_none=True)
...
class AuditLogSchema(SQLAlchemyAutoSchema):
    ...
    created_at = fields.DateTime(dump_only=True, format="iso")
    action = fields.String(required=True, validate=lambda x: x in VALID_AUDIT_ACTIONS)
...
class ErrorResponseSchema(Schema):
    ...
    timestamp = fields.DateTime(required=True, dump_default=lambda: datetime.now(timezone.utc))
...
class FileListRequestSchema(Schema):
    ...
    bucket = fields.String(required=True, validate=lambda x: x in VALID_BUCKET_TYPES)
...
class FileCopyRequestSchema(Schema):
    ...
    source_bucket = fields.String(required=True, validate=lambda x: x in VALID_BUCKET_TYPES)
    ...
    target_bucket = fields.String(required=True, validate=lambda x: x in VALID_BUCKET_TYPES)
...
class LockRequestSchema(Schema):
    ...
    lock_type = fields.String(validate=lambda x: x in VALID_LOCK_TYPES, load_default="edit")
...
class PresignedUrlRequestSchema(Schema):
    ...
    bucket_id = fields.String(required=True, validate=validate_uuid)
    logical_path = fields.String(required=True, validate=validate_path)
    expires_in = fields.Integer(load_default=3600, validate=lambda x: 300 <= x <= 86400)
...
class VersionListRequestSchema(Schema):
    ...
    file_id = fields.String(required=True, validate=validate_uuid)
    status = fields.String(validate=lambda x: x in VALID_VERSION_STATUSES, allow_none=True)
